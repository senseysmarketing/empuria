...
export function EditMemberPermissionsDialog({
  ...
  const save = async () => {
    ...
    for (const m of selected) if (!original.has(m)) toEnable.push(m);
    for (const m of original) if (!selected.has(m)) toDisable.push(m);
              ...
              {PERMISSION_GROUPS.map((group) => (
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    {group.comingSoon?.map((sub) => (
                      <div
                        key={sub.key}
                        className="flex items-center justify-between rounded-md border border-dashed border-admin-border/60 bg-admin-bg/40 px-3 py-2 opacity-70"
                      >
                        <div className="flex items-center gap-2 text-sm text-admin-ink-muted">
