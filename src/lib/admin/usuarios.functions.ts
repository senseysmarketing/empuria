...
export const listUsers = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    let q = supabaseAdmin.from("profiles").select("*").order("created_at", { ascending: false }).limit(1000);
    if (data.period === "7d") {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (authErr) break;
      authPage.users.forEach((u) => emails.set(u.id, { email: u.email ?? null, last_sign_in_at: u.last_sign_in_at ?? null }));
    ...
    return { items, total, totalActive, totalClub, newThisMonth, page: data.page, pageSize: data.pageSize };
  });
...
export const setUserBlocked = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { error: pErr } = await supabaseAdmin.from("profiles").update({ is_blocked: data.blocked }).eq("id", data.id);
...
export const forcePasswordReset = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { data: prof } = await supabaseAdmin.from("profiles").select("id").eq("id", data.id).maybeSingle();
...
export const changeUserEmail = createServerFn({ method: "POST" })
  .middleware([requireStaff])
  .inputValidator((d) => z.object({ id: z.string().uuid(), new_email: z.string().email().max(255) }).parse(d))
  .handler(async ({ data }) => {
    const { error } = await supabaseAdmin.auth.admin.updateUserById(data.id, { email: data.new_email, email_confirm: true });
    if (error) throw new Error(error.message);
    return { ok: true };
  });
